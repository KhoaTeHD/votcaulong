	...
	<div class="row">
        <div class="col-md-12">
            <section class="my-3 shadow-sm border border-light bg-white">
                ...
                <div class="row p-3">
                    <div class="col-md-9">
                        <ul class="news-nav nav  ">
                        <?php
                            $news_section = get_field('news_section', 'options');
                            if ($news_section['review_category']){
                                $exclude = [$news_section['review_category']->term_id];
                            }else{
                                $exclude = [];
                            }
                            $categories = get_categories(array(
                                'orderby' => 'name',
                                'order'   => 'ASC',
                                'hide_empty' => 1, // Hiển thị cả category không có bài viết
                                'exclude' => $exclude
                            ));

                            foreach ($categories as $category) {
                                $active_class = ''; // Mặc định không có class active
                                echo '<li class="nav-item"><a href="' . get_category_link($category->term_id) . '" class="nav-link ' . $active_class . '">' . $category->name . '</a></li>';
                            }
                            ?>
                        </ul>
                        <div class="row my-3">
                            <?php
                            $args = array(
                                'category_name'  => 'news', // Thay bằng slug của category bạn muốn lấy
                    ...
                    <div class="col-md-3">
                        <div class="link-title">
                            <h5><?php echo $news_section['review_category']->name??'Review'?></h5>
                            <a href="<?php echo get_category_link($news_section['review_category']->term_id)??'#'?>">Xem thêm</a>
                        </div>
                        <div class="row my-3 review-posts post-listing">
                        <?php
                            $args = array(
                                'category_name'  => $news_section['review_category']->slug??'review', 
                                'posts_per_page' => 3,
                            );
                                ...
                                while ($query->have_posts()) : $query->the_post(); ?>
                                    <article class="post-item">
                                        ...
                                        <div class="post-item-content">
                                            ...
                                            <span class="post-item-time"><?php echo human_time_diff(get_the_time('U'), current_time('timestamp')) . ' trước'; ?></span>
                                        </div>
                                    </article>
                                <?php endwhile;
                                wp_reset_postdata();
                    ...
                    $posts_per_page = 5;
                    $args = array(
                        'category_name' => 'news', // Slug của category cần đếm
                    ...
                    if ($query_pagination->have_posts()) : ?>
                        <section class="my-3 shadow-sm border border-light bg-white">
                            <div class="section-header">
                                <h3 class="title">Các Tin Tức Khác</h3>
                            </div>
                            <div class="row p-3">
